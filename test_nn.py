...
from train_nn import DeepNN, number_of_x_parameters, forward_solver, eReZmin, eReZmax, etabarMin, etabarMax, model_save_path, loss_save_path
...
iota_predicted_rc, elongation_predicted_rc, iL_predicted_rc = jnp.array([model.apply(params, [er, zs_base, eta_base]) for er in rc_array]).T
...
iota_predicted_zs, elongation_predicted_zs, iL_predicted_zs = jnp.array([model.apply(params, [rc_base, zs, eta_base]) for zs in zs_array]).T
...
iota_predicted_eta, elongation_predicted_eta, iL_predicted_eta = jnp.array([model.apply(params, [rc_base, zs_base, eta]) for eta in eta_array]).T
